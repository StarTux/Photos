...
final class AdminCommand implements CommandExecutor {
    ...
    static final class AdminException extends Exception {
        AdminException(final String message) {
            super(message);
        }
    }
    ...
    boolean onCommand(CommandSender sender, String cmd, String[] args) throws AdminException {
        ...
        case "list": {
            ...
            List<Photo> photos = plugin.findPhotos(uuid);
        ...
        case "name": {
            ...
            plugin.getDatabase().updatePhoto(photo);
            sender.sendMessage("Changed name of Photo #" + photo.getId()
                               + " to " + sb.toString() + ".");
            return true;
        }
        case "load": {
            ...
            plugin.downloadPhotoAsync(photo, url, true, (result) -> {
                    sender.sendMessage("Download URL (" + url + ") to Photo #"
                                       + photo.getId() + ": " + result.status);
                });
            return true;
        }
        case "color": {
    ...
    Photo parsePhoto(String arg) throws AdminException {
        ...
        Photo photo = plugin.findPhoto(id);
