...
public class PhotosMenu implements Listener {
        ...
        private Map<String, InventoryView> openMenus = new HashMap<String, InventoryView>();
        ...
        public void onInventoryClose(InventoryCloseEvent event) {
                openMenus.remove(event.getPlayer().getName());
        }

        @EventHandler(ignoreCancelled = true, priority = EventPriority.HIGH)
        public void onInventoryClick(InventoryClickEvent event) {
                if (openMenus.get(event.getWhoClicked().getName()) != null) {
                        event.setCancelled(true);
                        if (event.isShiftClick() && event.getRawSlot() < event.getView().getTopInventory().getSize()) {
                                Player player = (Player)event.getWhoClicked();
                                ItemStack item = event.getCurrentItem();
                                if (item != null && item.getType() == Material.MAP) {
                                        if (!plugin.economy.has(player.getName(), plugin.mapPrice)) {
                                                player.sendMessage("" + ChatColor.RED + "You don't have enough money");
                                                return;
                                        }
                                        if (!plugin.economy.withdrawPlayer(player.getName(), plugin.mapPrice).transactionSuccess()) {
        ...
        public void openMenu(Player player) {
                ...
                openMenus.put(player.getName(), view);
